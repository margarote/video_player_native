...
class VideoCacheManager: NSObject, AVAssetResourceLoaderDelegate {
    ...
    private let metadataQueue = DispatchQueue(label: "com.yourapp.VideoCacheMetadataQueue", attributes: .concurrent)

    override init() {
        // Configure cache com capacidade de 5GB
        cacheConfig = URLCache(memoryCapacity: 0, diskCapacity: maxCacheSize, diskPath: "VideoCache")
        super.init()
    ...
    func configureCaching(for url: URL) -> AVPlayerItem {
    ...
    private func saveCacheMetadata(for url: URL) {
        metadataQueue.async(flags: .barrier) {
            var metadata = UserDefaults.standard.dictionary(forKey: self.cacheMetadataKey) as? [String: TimeInterval] ?? [:]
            let currentTime = Date().timeIntervalSince1970

            metadata[url.absoluteString] = currentTime // Armazena o timestamp de último uso
            UserDefaults.standard.setValue(metadata, forKey: self.cacheMetadataKey)
    ...
    private func isCacheExpired(for url: URL, metadata: [String: TimeInterval]) -> Bool {
    ...
    func cleanExpiredOrExcessCache() {
        metadataQueue.async(flags: .barrier) {
            guard var metadata = UserDefaults.standard.dictionary(forKey: self.cacheMetadataKey) as? [String: TimeInterval] else { return }

            // Remova itens expirados
            let expiredKeys = metadata.filter { self.isCacheExpired(for: URL(string: $0.key)!, metadata: metadata) }.map { $0.key }
            for key in expiredKeys {
                if let url = URL(string: key) {
                    self.cacheConfig.removeCachedResponse(for: URLRequest(url: url))
                }
    ...
    private func enforceCacheLimit(metadata: [String: TimeInterval]? = nil) {
        metadataQueue.async(flags: .barrier) {
            var currentMetadata = metadata ?? (UserDefaults.standard.dictionary(forKey: self.cacheMetadataKey) as? [String: TimeInterval] ?? [:])

            while self.cacheConfig.currentDiskUsage > self.maxCacheSize, let oldest = currentMetadata.min(by: { $0.value < $1.value }) {
            ...
            UserDefaults.standard.setValue(currentMetadata, forKey: self.cacheMetadataKey)
        }
    }
    ...
    func resourceLoader(_ resourceLoader: AVAssetResourceLoader, shouldWaitForLoadingOfRequestedResource loadingRequest: AVAssetResourceLoadingRequest) -> Bool {
        ...
        let task = session.dataTask(with: cacheRequest) { data, response, error in
            if let error = error {
            ...
            if let data = data, let response = response {
                ...
                self.cacheConfig.storeCachedResponse(cachedResponse, for: cacheRequest)
                loadingRequest.dataRequest?.respond(with: data)
                loadingRequest.finishLoading()
            } else {
                loadingRequest.finishLoading(with: NSError(domain: "No Data", code: -1, userInfo: nil))
            }
        }
        task.resume()
